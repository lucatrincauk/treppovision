...
import { Users, UserCircle, Edit, Music2, Star, ThumbsDown, Shirt, Lock, BadgeCheck, Award, ListChecks } from "lucide-react";
...
interface SelectedNationDisplayProps {
  ...
  allNations: Nation[]; // Added to resolve missing artist/song on podium
}
...
const SelectedNationDisplay = React.memo(({
  ...
  const nationData = nation; // Use the nation object directly
  ...
  } else if (nationData?.ranking && nationData.ranking > 0) { // "Pronostici TreppoVision" items
    rankTextForDisplay = `(${nationData.ranking}°)`;
  }

  const mainContainerClasses = cn(
    "px-2 py-1",
    label ? "flex-col items-start gap-1 py-1.5 sm:flex-row sm:items-center sm:gap-1.5 sm:py-1" : "flex items-center gap-1.5",
          ...
          <Link href={`/nations/${nationData.id}`} className="group flex-grow flex items-center gap-1">
            ...
            <div className="flex flex-col text-left flex-grow min-w-0">
              <div className="flex items-center">
                ...
                {!label && nationData?.ranking && [1,2,3].includes(nationData.ranking) && <MedalIcon rank={nationData.ranking} isEurovisionRank={true} />}
                ...
                {rankTextForDisplay && label && categoryRank && [1,2,3].includes(categoryRank) && (
                   <span className={cn("text-xs text-muted-foreground group-hover:text-primary/80 ml-0.5 whitespace-nowrap")}>
                     {rankTextForDisplay}
                   </span>
                )}
              </div>
              {(isOwnTeamCard || isLeaderboardPodiumCard) && artistAndSongForDisplay && label && (
      ...
      <div className="flex-grow">
        ...
        >
         ...
         {nationData?.artistName && nationData?.songTitle && (
            <p className="text-[10px] text-muted-foreground truncate max-w-[180px] sm:text-xs" title={`${nationData.artistName} - ${nationData.songTitle}`}>
                {nationData.artistName} - {nationData.songTitle}
            </p>
        )}
        </Link>
...
const CategoryPickDisplayDetailPodium = React.memo(({ detail, allNations }: CategoryPickDisplayDetailPodiumProps) => {
  ...
  if (detail.actualCategoryRank && detail.actualCategoryRank > 0) {
      ...
      // Only show rank text if it's a top 3 pick for medal display consistency
      if ([1,2,3].includes(detail.actualCategoryRank)) {
              ...
              >
                  <div className="flex items-center gap-1">
                   ...
                   {pickedNationFullDetails?.artistName && pickedNationFullDetails?.songTitle && (
                      <p className="text-[10px] text-muted-foreground truncate max-w-[180px] sm:text-xs" title={`${pickedNationFullDetails.artistName} - ${pickedNationFullDetails.songTitle}`}>
                          {pickedNationFullDetails.artistName} - {pickedNationFullDetails.songTitle}
                      </p>
                  )}
              </Link>
...
interface TeamListItemProps {
  ...
  nations: Nation[];
  allNations?: Nation[]; // Made optional for contexts where it's not provided, but required for podium.
  nationGlobalCategorizedScoresMap: Map<string, NationGlobalCategorizedScores>;
  isOwnTeamCard?: boolean;
  disableEdit?: boolean;
}

export function TeamListItem({ team, nations, allNations = [], nationGlobalCategorizedScoresMap, isOwnTeamCard = false, disableEdit = false }: TeamListItemProps) {
  const { user } = useAuth();
  ...
  const getNationDetailsById = (id?: string): Nation | undefined => {
    if (!id || !nations || nations.length === 0) return undefined;
          ...
          <>
            ...
            {founderNationsDetails.map((nation, index) => (
              <SelectedNationDisplay
                ...
                isLeaderboardPodiumCard={!!team.rank}
                allNations={allNations}
              />
            ))}
            <p className="text-xl font-bold text-secondary mt-4 pt-3 border-t border-border/30 mb-1.5">
